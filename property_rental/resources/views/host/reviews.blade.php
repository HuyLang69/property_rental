...
@section('title', 'Reviews — NestAway')
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10">
        <div class="bg-ink text-white rounded-2xl p-6">
            ...
            <div class="flex items-baseline gap-2">
                <p class="font-display text-4xl font-bold">{{ number_format($avgRating, 1) }}</p>
        ...
        <div class="bg-white border border-fog rounded-2xl p-6">
            ...
            <p class="font-display text-4xl font-bold text-ink">{{ $reviews->count() }}</p>
        </div>
        <div class="bg-white border border-fog rounded-2xl p-6">
            <p class="text-xs uppercase tracking-widest text-silver mb-2">5-star reviews</p>
            <p class="font-display text-4xl font-bold text-ink">{{ $reviews->where('rating', 5)->count() }}</p>
            <p class="text-xs text-silver mt-1">{{ $reviews->count() > 0 ? round($reviews->where('rating', 5)->count() / $reviews->count() * 100) : 0 }}% of total</p>
        </div>
    </div>
    @endif
        ...
        <div class="flex flex-col gap-4">
            ...
            <div class="bg-white border border-fog rounded-2xl p-5 flex flex-col gap-4">
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            <p class="text-sm font-semibold text-ink">
                                {{ $review->user->first_name }} {{ substr($review->user->last_name, 0, 1) }}.
                            </p>
                            <p class="text-xs text-silver mt-0.5">{{ $review->created_at->format('M d, Y') }}</p>
                        </div>
                    </div>

                    <div class="flex items-center gap-1 shrink-0">
                        @for ($i = 1; $i <= 5; $i++)
                        <svg class="w-3.5 h-3.5 {{ $i <= $review->rating ? 'text-ink' : 'text-fog' }}"
                ...
                <div class="flex items-center gap-2 pt-1 border-t border-fog">
                    ...
                    <a href="{{ route('listings.show', $review->listing_id) }}"
