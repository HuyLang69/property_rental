...
@section('title', 'My Trips — NestAway')
...
<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <div class="flex gap-1 mb-6 border-b border-fog overflow-x-auto">
        ...
        <a href="{{ route('dashboard.bookings', ['filter' => $key]) }}"
           class="whitespace-nowrap px-4 py-2.5 text-sm font-medium transition-colors border-b-2 -mb-px
                  {{ $filter === $key ? 'border-ink text-ink' : 'border-transparent text-silver hover:text-ink' }}">
            {{ $label }}
        </a>
        @endforeach
    </div>
    ...
    @else
        <div class="flex flex-col gap-4">
            ...
            <div class="bg-white border border-fog rounded-2xl overflow-hidden flex flex-col sm:flex-row">
                ...
                <div class="flex flex-1 flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-5">
                    <div class="flex-1 min-w-0">
                        ...
                        <h3 class="font-semibold text-ink leading-snug">{{ $booking->listing->title }}</h3>
                    ...
                    <div class="flex flex-col items-start sm:items-end gap-3 shrink-0">
                        ...
                        <div class="flex flex-wrap gap-2">
                            ...
                            {{-- Review button — only for confirmed past stays without a review --}}
                            @if ($booking->status === 'confirmed' && $booking->check_out->isPast() && !$booking->hasReview())
                            ...
                            <button type="button"
                                    onclick="openModal('modal-{{ $booking->id }}')"
            ...
            {{-- Cancel modal --}}
            @if ($booking->status !== 'cancelled')
            <div id="modal-{{ $booking->id }}" class="fixed inset-0 z-50 hidden items-center justify-center px-4">
                <div class="absolute inset-0 bg-ink/40 backdrop-blur-sm"
                     onclick="closeModal('modal-{{ $booking->id }}')"></div>
