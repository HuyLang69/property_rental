<?php $__env->startSection('title', 'NestAway — Find Your Perfect Stay'); ?>
...
<section class="relative min-h-[90vh] flex items-center justify-center overflow-hidden bg-ink">
    ...
    <div class="relative z-10 text-center text-white px-4 sm:px-6 max-w-4xl mx-auto py-20">
        ...
        <div class="fade-up d5 flex flex-wrap justify-center gap-2 mt-6">
            <a href="<?php echo e(route('listings.index', ['search' => 'Lisbon'])); ?>"  class="text-xs text-silver border border-white/20 rounded-full px-3 py-1 hover:bg-white/10 transition-colors">Lisbon</a>
            <a href="<?php echo e(route('listings.index', ['search' => 'Porto'])); ?>"   class="text-xs text-silver border border-white/20 rounded-full px-3 py-1 hover:bg-white/10 transition-colors">Porto</a>
            <a href="<?php echo e(route('listings.index', ['search' => 'Lagos'])); ?>"   class="text-xs text-silver border border-white/20 rounded-full px-3 py-1 hover:bg-white/10 transition-colors">Lagos</a>
            <a href="<?php echo e(route('listings.index', ['search' => 'Sintra'])); ?>"  class="text-xs text-silver border border-white/20 rounded-full px-3 py-1 hover:bg-white/10 transition-colors">Sintra</a>
            <a href="<?php echo e(route('listings.index', ['search' => 'Cascais'])); ?>" class="text-xs text-silver border border-white/20 rounded-full px-3 py-1 hover:bg-white/10 transition-colors">Cascais</a>
        </div>
    </div>
...
<section id="categories" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 pb-8">
    <div class="flex items-center justify-between mb-8">
        ...
        <a href="<?php echo e(route('listings.index')); ?>" class="hidden sm:flex items-center gap-1 text-sm font-medium text-slate hover:text-ink transition-colors">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
        ...
        <a href="<?php echo e(route('listings.index', ['type' => 'apartment'])); ?>" class="lift group flex flex-col items-center justify-center gap-3 bg-white border border-fog rounded-2xl py-7 px-4 text-center hover:border-ink transition-colors">
        ...
        <a href="<?php echo e(route('listings.index', ['type' => 'house'])); ?>" class="lift group flex flex-col items-center justify-center gap-3 bg-white border border-fog rounded-2xl py-7 px-4 text-center hover:border-ink transition-colors">
            <div class="w-11 h-11 rounded-full bg-cream group-hover:bg-ink flex items-center justify-center transition-colors">
                <svg class="w-5 h-5 text-ink group-hover:text-white transition-colors" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
        ...
        <a href="<?php echo e(route('listings.index', ['type' => 'studio'])); ?>" class="lift group flex flex-col items-center justify-center gap-3 bg-white border border-fog rounded-2xl py-7 px-4 text-center hover:border-ink transition-colors">
            <div class="w-11 h-11 rounded-full bg-cream group-hover:bg-ink flex items-center justify-center transition-colors">
                <svg class="w-5 h-5 text-ink group-hover:text-white transition-colors" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
        ...
        <a href="<?php echo e(route('listings.index', ['type' => 'villa'])); ?>" class="lift group flex flex-col items-center justify-center gap-3 bg-white border border-fog rounded-2xl py-7 px-4 text-center hover:border-ink transition-colors">
            <div class="w-11 h-11 rounded-full bg-cream group-hover:bg-ink flex items-center justify-center transition-colors">
                <svg class="w-5 h-5 text-ink group-hover:text-white transition-colors" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
        ...
        <a href="<?php echo e(route('listings.index', ['type' => 'room'])); ?>" class="lift group flex flex-col items-center justify-center gap-3 bg-white border border-fog rounded-2xl py-7 px-4 text-center hover:border-ink transition-colors">
            <div class="w-11 h-11 rounded-full bg-cream group-hover:bg-ink flex items-center justify-center transition-colors">
                <svg class="w-5 h-5 text-ink group-hover:text-white transition-colors" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
...
<section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
        ...
        <a href="<?php echo e(route('listings.show', $listing->id)); ?>" class="lift group bg-white rounded-2xl overflow-hidden border border-fog flex flex-col">
            <div class="img-zoom relative" style="aspect-ratio:4/3;">
                <?php if($listing->coverImage): ?>
                    <img src="<?php echo e($listing->coverImage->path); ?>" alt="<?php echo e($listing->title); ?>" class="w-full h-full object-cover" loading="lazy" />
                <?php else: ?>
                ...
                <?php endif; ?>
                <span class="absolute top-3 left-3 bg-ink/75 backdrop-blur-sm text-white text-[10px] uppercase tracking-widest font-semibold rounded-full px-2.5 py-1"><?php echo e($listing->type); ?></span>
            </div>
            <div class="flex flex-col flex-1 p-5 gap-3">
                <div class="flex items-start justify-between gap-2">
                    <div class="flex-1 min-w-0">
                        <h3 class="font-semibold text-ink text-[0.95rem] leading-snug truncate"><?php echo e($listing->title); ?></h3>
                    ...
                    <div class="flex items-center gap-1 shrink-0">
                        ...
                        <span class="text-xs font-semibold text-ink"><?php echo e(number_format($listing->reviews_avg_rating, 1)); ?></span>
                        <span class="text-xs text-silver">(<?php echo e($listing->reviews->count()); ?>)</span>
                    </div>
                    <?php endif; ?>
                </div>
                <p class="text-xs text-slate"><?php echo e($listing->bedrooms); ?> bed<?php echo e($listing->bedrooms > 1 ? 's' : ''); ?> &middot; Up to <?php echo e($listing->max_guests); ?> guests</p>
                <div class="mt-auto flex items-center justify-between">
                    <p><span class="text-base font-bold">$<?php echo e(number_format($listing->price_per_night / 100, 0)); ?></span><span class="text-xs text-silver font-normal"> / night</span></p>
    ...
    <div class="sm:hidden mt-8 text-center">
        <a href="<?php echo e(route('listings.index')); ?>" class="inline-flex items-center gap-2 text-sm font-medium border border-fog rounded-full px-6 py-3 hover:border-ink transition-colors">
...
<section class="bg-white border-t border-fog py-16">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between mb-10">
            ...
            <a href="<?php echo e(route('listings.index', ['sort' => 'newest'])); ?>" class="hidden sm:flex items-center gap-1 text-sm font-medium text-slate hover:text-ink transition-colors">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            <?php $__currentLoopData = $recent; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $listing): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <a href="<?php echo e(route('listings.show', $listing->id)); ?>" class="lift group bg-cream rounded-2xl overflow-hidden border border-fog flex flex-col">
                ...
                <div class="p-4 flex flex-col gap-1 flex-1">
                    <h3 class="text-sm font-semibold text-ink leading-snug line-clamp-2 group-hover:text-carbon transition-colors"><?php echo e($listing->title); ?></h3>
                    <p class="text-xs text-silver"><?php echo e($listing->city); ?> &middot; <?php echo e(ucfirst($listing->type)); ?></p>
                    <p class="mt-auto pt-3 text-sm font-bold text-ink">
                        $<?php echo e(number_format($listing->price_per_night / 100, 0)); ?><span class="text-xs font-normal text-silver"> / night</span>
                    </p>
                </div>
            </a>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
...
<section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
    <div class="bg-ink rounded-3xl px-8 sm:px-14 py-14 flex flex-col sm:flex-row items-center justify-between gap-8 relative overflow-hidden">
        ...
        <div class="relative flex flex-col gap-3 shrink-0 w-full sm:w-auto">
            <a href="<?php echo e(url('/host')); ?>" class="bg-white text-ink font-semibold text-sm rounded-full px-8 py-3.5 hover:bg-fog transition-colors text-center">
                Become a Host &rarr;
            </a>
        </div>
    </div>
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\User\Documents\GitHub\property-rental-laravel\property_rental\resources\views/home.blade.php ENDPATH**/ ?>